...
lowbRed=np.array([2, 143, 28])
upbRed=np.array([16, 241, 201])

lowbGreen=np.array([35, 147, 55])
upbGreen=np.array([71, 255, 137])
...
def datchikHSV1():
    global frame, red, green, maxR, maxG

    datb1 = frame[pos3[2]:pos3[3], pos3[0]:pos3[1]]
    dat1 = cv2.GaussianBlur(datb1, (5, 5), cv2.BORDER_DEFAULT)
    hsv1 = cv2.cvtColor(dat1.copy(), cv2.COLOR_BGR2HSV)
    hsv2 = cv2.cvtColor(dat1.copy(), cv2.COLOR_BGR2HSV)
    maskdR = cv2.inRange(hsv1, lowbRed, upbRed)
    maskdG = cv2.inRange(hsv2, lowbGreen, upbGreen)

    _, contoursdR, _ = cv2.findContours(maskdR, cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE)
    _, contoursdG, _ = cv2.findContours(maskdG, cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE)

    for contorb1 in contoursdR:
        x, y, w, h = cv2.boundingRect(contorb1)
        a1 = cv2.contourArea(contorb1)
        if a1 > 500:
            red =True
    ...
    cv2.rectangle(frame, (pos3[0], pos3[2]), (pos3[1], pos3[3]), (255, 255, 0), 2)
